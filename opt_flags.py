...
COMBOS_DB = Path('passes_combos.db')

TESTIN_BC = 'hello.bc' # Bitcode to use as input for opt
TESTOUT_LL = 'hello2.ll' # opt output bitcode
TESTOUT_PTX = 'hello2.ptx' # output ptx
...
class Version(object):
    '''Very simple class representing a distinct program version of TESTIN_BC.
    ...
    def _optimize(self):
        ...
        cmd_opt = [OPT, '-O2', '--mcpu=sm_60'] + passes_txt + [TESTIN_BC, '-o', TESTOUT_LL, '-S']
        try:
...
def save_to_db(combos):
    with sqlite3.connect(COMBOS_DB) as conn:
        conn.execute(
        ...
        for v, combo in combos.items():
            ...
            conn.execute('INSERT INTO versions VALUES (?, ?, ?)', (passes_txt, v.ir, v.ptx))
